...
func _ready()->void:
	...
	var initial_leaf_count:int=harvest_target.leaves.size()
	for i in range(120): harvest_target.simulate(.1)
	assert(harvest_target.leaves.size()>initial_leaf_count)
	assert(harvest_target.scale.is_equal_approx(Vector3.ONE))
	assert(harvest_target.leaves[0].age>harvest_target.leaves[-1].age)
	var grown_leaf_count:int=harvest_target.leaves.size()
	...
	await get_tree().create_timer(1.2).timeout
	assert(game.plants.size()==initial_count)
	print("SMOKE_OK one_out_one_in plants=",game.plants.size()," leaves=",grown_leaf_count," root_scale=1 species=",species_id)
	get_tree().quit()
