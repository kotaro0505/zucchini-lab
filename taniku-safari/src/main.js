...
const assets={jeep:image('assets/images/jeep/safari-jeep.png'),background:image('assets/images/background/mexican-trail.png'),atlas:image('assets/images/succulents/specimen-atlas.png')};
const audio=new AudioSystem();
const ui={update(g){$('#score').textContent=Math.floor(g.score).toString().padStart(5,'0');$('#rare').textContent=g.rare;const n=Math.min(7,Math.ceil(g.speed*2.7));$('#speedBars').innerHTML=Array.from({length:7},(_,i)=>`<i class="${i<n?'on':''}" style="height:${8+i*2}px"></i>`).join('')},toast(text,kind){const t=$('#toast');t.textContent=text;t.className='';void t.offsetWidth;t.classList.add('show');if(kind==='big')t.classList.add('big');if(kind!=='normal'){$('#flash').className='';void $('#flash').offsetWidth;$('#flash').className='on'}},finish(g){$('#finalScore').textContent=Math.floor(g.score).toLocaleString();$('#resultTitle').textContent=g.rare?'レア個体を発見！':'サファリ終了';$('#haul').innerHTML=Object.entries(g.haul).map(([id,n])=>`<span>${id.replaceAll('_',' ')} ×${n}</span>`).join('')||'<span>次はきっと出会える</span>';$('#hud').hidden=true;show('result')}};
