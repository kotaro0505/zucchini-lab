export class GameUI{
 constructor(){this.score=document.querySelector('#score');this.high=document.querySelector('#high-score');this.multi=document.querySelector('#multiplier');this.balls=document.querySelector('#balls');this.lcd=document.querySelector('#lcd');this.lcdMain=document.querySelector('#lcd-main');this.lcdSub=document.querySelector('#lcd-sub');this.msg=document.querySelector('#message');this.overlay=document.querySelector('#start-overlay');this.start=document.querySelector('#start-button');this.left=document.querySelector('#left-control');this.right=document.querySelector('#right-control')}
 ...
 lighting(light){const s=light.snapshot();document.documentElement.style.setProperty('--awake',s.energy.toFixed(3));document.documentElement.style.setProperty('--pulse',Math.max(s.pulse,s.reward,s.roulette).toFixed(3));document.body.dataset.lightStage=String(s.stage)}
}
